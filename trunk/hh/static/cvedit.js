    ...
    $.get( '/positions/info' ).success( function(positions) {
        ...
        $.each( positions, function() {
            $('<div id="pos_field'+this.id+'">'+
              ...
              '<hr /><input id = "denied_pos'+this.id+'" type = "button" style="width:35%;" value = "Отказать" class="ui-button ui-widget ui-state-default ui-corner-all ui-state-hover" aria-disabled="false"/>'+
            ...
            $('#denied_pos'+this.id).click(function() {
                if (confirm("Вы подтверждаете отказ? Отказе данная вакансия больше не будет показываться в списке.")) {
