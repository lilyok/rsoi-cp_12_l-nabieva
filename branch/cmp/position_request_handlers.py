...
import datetime
from database import IsTrip, Schedule, Education, Universities, Position, Lang, LangLevel, EduLang, Industry, Resume
...
class PositionRequestHandlers(object):
    ...
    def resume(self):
        resume = []
        with self.__database.session() as database_session:
            for instance in database_session.query(Resume).order_by(Resume.id_pos).all():
                print instance.name
                print instance.id_pos               
                resm = {}
                resm['id'] = instance.id   
                resm['status'] = instance.status       
                resm['name'] = instance.name
                resm['skills'] = instance.skills   
                resm['post'] = instance.post                                
                resm['wage'] = instance.wage
                resm['exp'] = instance.exp
                resm['schd'] = instance.schd                
                resm['trip'] = instance.trip
                resm['lan'] = instance.lan                    
                resm['educ'] = instance.educ 
                resm['email'] = instance.email                             

                resm['position'] = database_session.query(Position).filter(instance.id_pos == Position.id).one().name
    ...
    def open_position(self, num_edu, num_lang, pid=None, id_sphere=None, id_schedule=None, id_trip=None, pos_name='', info = '', min_wage=None, max_wage=None, exp=None,lang_ids=None, llev_ids=None, univ_ids=None, specialities=None):
        ...
        with self.__database.session() as database_session:
            print id_sphere
            mail_address = database_session.query(Industry).get(id_sphere).e_mail
    ...
    def accept(self, id):
        ...
        sender_address = cherrypy.config['mail_address']  
        with self.__database.session() as database_session:
            posres = database_session.query(Resume).get(id)
            print "status: ", posres.status
            if posres.status < 0 and abs(posres.status) % 2 != 0:
                posres.status -= 1
            content = {
                'action': 'accept_resm',
                'id_pos': posres.id_pos,
                'id_resm': posres.resm_id,
                'status': posres.status
            }                
            send_as_json(sender_address, posres.e_mail, content)    
            database_session.commit()
            print posres.status
            return posres.status        
    ...
    def deleted(self, id):
        ...
        with self.__database.session() as database_session:
            posres = database_session.query(Resume).get(id)
            database_session.delete(posres)
    ...
    def close(self, id):
        ...
        sender_address = cherrypy.config['mail_address']
        with self.__database.session() as database_session:
            pos = database_session.query(Position).get(id)
            email = database_session.query(Industry).get(pos.id_inds).e_mail
            content = {
                'action': 'close',
                'id': id            
            }
            send_as_json(sender_address, email, content)
